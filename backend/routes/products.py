...
async def get_product_detail(product_id: str, user=Depends(get_current_user)):
    ...
    for b in all_branches:
        inv = await db.inventory.find_one({"product_id": product_id, "branch_id": b["id"]}, {"_id": 0})
        qty = inv["quantity"] if inv else 0
        branch_stock[b["id"]] = {"branch_name": b["name"], "quantity": qty}
        total_stock += qty
    coming = await db.purchase_orders.aggregate([
        {"$match": {"status": {"$in": ["ordered", "draft"]}}}, {"$unwind": "$items"},
        {"$match": {"items.product_id": product_id}}, {"$group": {"_id": None, "t": {"$sum": "$items.quantity"}}}
    ]).to_list(1)
    reserved = await db.sales.aggregate([
        {"$match": {"status": "reserved"}}, {"$unwind": "$items"},
        {"$match": {"items.product_id": product_id}}, {"$group": {"_id": None, "t": {"$sum": "$items.quantity"}}}
    ]).to_list(1)
    movements = await db.inventory_movements.find({"product_id": product_id}, {"_id": 0}).sort("created_at", -1).limit(50).to_list(50)
    return {
        "product": product, "branch_stock": branch_stock, "total_stock": total_stock,
        "coming": coming[0]["t"] if coming else 0, "reserved": reserved[0]["t"] if reserved else 0,
        "repacks": repacks, "vendors": vendors, "branches": all_branches, "movements": movements,
    }

@router.get("/{product_id}/vendors")
...
async def add_product_vendor(product_id: str, data: dict, user=Depends(get_current_user)):
    ...
    vendor = {
        ...
        "vendor_name": data.get("vendor_name", ""), "vendor_sku": data.get("vendor_sku", ""),
        "lead_time_days": int(data.get("lead_time_days", 0)), "min_order_qty": float(data.get("min_order_qty", 0)),
        "created_at": now_iso(),
    }
...
@router.delete("/{product_id}/vendors/{vendor_id}")
...
@router.get("/{product_id}/order-history")
async def get_product_order_history(product_id: str, user=Depends(get_current_user)):
    sales = await db.sales.find({"items.product_id": product_id}, {"_id": 0}).sort("created_at", -1).limit(30).to_list(30)
    purchases = await db.purchase_orders.find({"items.product_id": product_id}, {"_id": 0}).sort("created_at", -1).limit(30).to_list(30)
    history = []
    for s in sales:
        for it in s.get("items", []):
            if it.get("product_id") == product_id:
                history.append({"type": "sale", "date": s.get("created_at"), "ref": s.get("sale_number"),
                                "customer": s.get("customer_name"), "qty": it.get("quantity"), "price": it.get("price")})
    for p in purchases:
        for it in p.get("items", []):
            if it.get("product_id") == product_id:
                history.append({"type": "purchase", "date": p.get("created_at"), "ref": p.get("po_number"),
                                "vendor": p.get("vendor"), "qty": it.get("quantity"), "price": it.get("unit_price")})
